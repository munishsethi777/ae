...
public class QuestionDataStore implements QuestionDataStoreI, RowMapper {
	...
	private final static String SELECT_BY_SEQ = SELECT
			+ "where questions.seq = ?";
	...
	private final static String FIND_QUESTIONS_BY_GAME = "select questions.*,questionanswers.seq as answerSeq,questionanswers.title as answerTitle, "
			+ "questionanswers.iscorrect as iscorrect from questions "
	...
	public List<Questions> findSelectedByGameSeq(long gameSeq) {
		...
		return (List<Questions>) persistenceMgr.executePSQuery(
				FIND_QUESTIONS_SELECTED_IN_GAME, params, this);
	}
	...
	public List<Questions> findAvailableByGameSeq(long gameSeq) {
		Object[] params = new Object[] { gameSeq };
		return (List<Questions>) persistenceMgr.executePSQuery(
				FIND_QUESTIONS_AVAILABLE_FOR_GAME, params, this);
	}
	...
	public void Save(Questions questions) {
		...
		try {
			...
			questionAnswerMgr
					.addQuestionAnswers(questions.getQuestionAnswers());
		} catch (Exception e) {
	...
	public Questions findBySeq(long seq) {
		...
		List<Questions> questions = getQuestionsAndAnswers(SELECT_BY_SEQ,
				params);
			...
			public Object mapRow(ResultSet rs) throws SQLException {
				try {
					...
					if (map.get(question.getSeq()) == null) {
						...
						Questions mapQuestion = (Questions) map.get(question
								.getSeq());
						mapQuestion.getQuestionAnswers().add(
								question.getQuestionAnswers().get(0));
					}
				} catch (Exception e) {
					logger.error(e);
				}
