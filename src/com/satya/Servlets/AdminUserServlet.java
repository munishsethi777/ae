			...
			if (getLoggedinAdmin(request) == null) {
						...
						.getImageMgr();
				{
					...
					// PROJECTS DROPDOWN CHANGE
						...
						JSONObject json = userGroupMgr.updateUserGroup(request,
								response);
						response.getWriter().write(json.toString());
					}
					...
					// campaign UI methods
						...
						JSONObject json = campaignMgr.getFullJSON(request,
								response);
						response.getWriter().write(json.toString());
						...
						JSONArray jsonarr = gameMgr.removeQuestionFromGame(request);
						response.getWriter().write(jsonarr.toString());
					}
				}
